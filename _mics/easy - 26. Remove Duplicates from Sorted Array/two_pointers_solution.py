...
ADDITIONAL:
Not clear description - point is to count unique values. That's all.

"""

def removeDuplicates(nums):
    r = 0
    # if iterating through list with 2 pointers
    # a nice solution is one pointer being 0 and faster pointer i in range(len(list))
    for i in range(1, len(nums)):
        # if unique value is found
        if nums[r] != nums[i]:
            # move pointer in the list - also counter of unique values since it
            # only increases when a unique value found and starts at 0
            r += 1
            # replace the value we are looking for
            nums[r] = nums[i]
        i += 1
    return r+1
